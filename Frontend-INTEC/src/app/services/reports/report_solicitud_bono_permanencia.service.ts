...
export interface SolicitudBonoPermanenciaData {
...
export class ReportSolicitudBonoPermanenciaService {

  async generate(data: SolicitudBonoPermanenciaData): Promise<void> {
    const doc = new jsPDF({ orientation: 'portrait', unit: 'mm', format: 'a4' });
    const logoBase64 = await this.loadLogoBase64();

    // 3 copias por hoja — cada bloque ocupa ~94mm
    const slotHeight = 94;
    for (let i = 0; i < 3; i++) {
      const offsetY = i * slotHeight;
      this.drawCopy(doc, data, logoBase64, offsetY);
      // Línea punteada azul entre copias
      if (i < 2) {
    ...
    doc.save(`SolicitudBonoPermanencia_${today}.pdf`);
  }
  ...
  private drawCopy(
    doc: jsPDF,
    data: SolicitudBonoPermanenciaData,
    ...
    // ── Encabezado empresa (centro-derecha del logo) ──────────────────────────
    const hx = lm + 20;
    ...
    doc.text('SOLICITUD DE BONO POR PERMANENCIA', lm + pw / 2, y, { align: 'center' });
    y += 2;

    // Línea azul bajo el título
    doc.setDrawColor(...blue);
    doc.setLineWidth(0.4);
    const titleW = doc.getTextWidth('SOLICITUD DE BONO POR PERMANENCIA');
    doc.line(lm + pw / 2 - titleW / 2, y, lm + pw / 2 + titleW / 2, y);
    y += 4;

    // ── DATOS DE EMPLEADO — header gris redondeado ────────────────────────────
  ...
  y += 5 + 6; // puedes ajustar el gap si usas una variable
    ...
    doc.setFontSize(6.5);
    doc.setTextColor(...blue);
    doc.text('Aprobada: Jefe directo', sig1X + sigW / 2, y + 3.5, { align: 'center' });
    doc.text('Encargado de obra / Residente', sig1X + sigW / 2, y + 6.5, { align: 'center' });
    doc.setDrawColor(...gray);
    doc.setLineWidth(0.3);
    doc.line(sig1X + 4, y + 10, sig1X + sigW - 4, y + 10);
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(6);
    doc.setTextColor(...blue);
    doc.text('Firma', sig1X + sigW / 2, y + 13, { align: 'center' });
    ...
    doc.setFontSize(6.5);
    doc.setTextColor(...blue);
    doc.text('Enterado', sig2X + sigW / 2, y + 3.5, { align: 'center' });
    doc.setDrawColor(...gray);
    doc.setLineWidth(0.3);
    doc.line(sig2X + 4, y + 10, sig2X + sigW - 4, y + 10);
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(6);
    doc.setTextColor(...blue);
    doc.text('Firma de R.H.', sig2X + sigW / 2, y + 13, { align: 'center' });
  }
}
